...
func searchMailByEmbedding(ctx context.Context, db *sql.DB, userID string, q mailQuery, question string, limit int) ([]insightHit, error) {
	if db == nil || !mailVectorsEnabled {
		return nil, nil
	}
	if limit <= 0 {
...
func mergeInsightHits(base, extra []insightHit) []insightHit {
	seen := map[string]bool{}
	var out []insightHit
	add := func(h insightHit) {
		key := h.ID
		if key == "" {
			key = insightHitKey(h)
		}
...
func insightWantsRAG(text string, q mailQuery, hits []insightHit) bool {
